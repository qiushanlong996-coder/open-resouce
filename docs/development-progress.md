...
## 2026-08-07：自拟需求第三批（详情相关推荐 + 复制项目简介）

1. **项目详情「相关推荐」**：按同分类拉取其他已发布项目（排除当前），
   详情页底部横向卡片，点击直达对应项目。
2. **复制项目简介**：「更多操作」菜单新增「复制项目简介」，一键复制
   名称 + 简介 + 仓库地址，便于作者在社区/文档中分享。

### 验证与部署

- 前端构建通过；发布 `20260807-related-copy-summary`（330 文件）。

## 2026-08-07：移动端全站布局审计与修复

### 审计方法

- 用无头浏览器（CDP DOM 测量，非截图）在 390px 视口逐页打开
  首页、项目详情（文档/代码/下载/概览四个页签）、搜索面板、开放 API 文档、
  登录弹窗、管理台、作者中心、通知浮层，检测文档级横向溢出与越界元素。

### 发现并修复

1. **首页分类胶囊行溢出**：`.filter-row` 单行不换行，整行被裁掉
   （实测 738px vs 350px）→ 移动端允许换行。
2. **文章阅读页工具栏溢出**（用户反馈的重点）：工具栏（阅读设置/链接/下载）
   挤到 416px 超出视口 → 工具栏与操作按钮组允许换行；
   文档区从 416px 回到 390px。
3. **详情操作按钮区兜底**：收藏/关注/分享/举报/更多等按钮多时允许换行；
   详情元信息（仓库地址/许可证）支持长串断行。
4. **深链支持 `?tab=`**：`/projects/{slug}?tab=代码预览` 可直接打开对应页签，
   便于分享与审计。

### 验证

- 修复后复测：首页、文档阅读页文档级 `scrollWidth == viewport`（390），
